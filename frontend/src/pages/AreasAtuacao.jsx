...
const AreasAtuacao = () => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1000px] mx-auto animate-fade-in text-theme-strong">
      <div className="flex items-center justify-between border-b border-aldebaran-border pb-4">
        <div>
          <h1 className="text-2xl font-extrabold flex items-center gap-3 font-title">
            <Settings className="text-aldebaran-gold w-8 h-8" />
      ...
      {user?.role === 'admin' && (
        <form onSubmit={handleCreate} className="bg-aldebaran-gray border border-aldebaran-border p-5 flex flex-col sm:flex-row items-end gap-4 shadow-sm">
          <div className="flex-1 w-full">
            <label className="text-xs font-semibold text-theme-weak block mb-1">Nome da Nova Área *</label>
            <input 
              type="text"
              required
              ...
              className="w-full p-2.5 bg-aldebaran-dark border border-aldebaran-border text-sm text-theme-strong focus:outline-none focus:border-aldebaran-gold"
            />
          </div>
          <div>
            <label className="text-xs font-semibold text-theme-weak block mb-1">Cor Visual</label>
            <input 
              type="color"
              value={form.cor_visual}
              onChange={(e) => setForm({...form, cor_visual: e.target.value})}
              className="w-12 h-[42px] p-1 bg-aldebaran-dark border border-aldebaran-border cursor-pointer"
            />
          </div>
          <button
            type="submit"
            className="px-6 py-[11px] bg-aldebaran-gold hover:bg-aldebaran-goldDark text-white font-bold text-sm transition-all shadow-sm whitespace-nowrap"
          >
            Adicionar Área
          </button>
        </form>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {areas.length > 0 ? (
            ...
            <div key={area.id} className="bg-aldebaran-dark border border-aldebaran-border p-4 flex items-center justify-between hover:border-aldebaran-gold/50 transition-all">
              ...
              {user?.role === 'admin' && (
                <button
                  ...
                  className="p-2 hover:bg-rose-500/10 text-theme-weak hover:text-rose-500 transition"
                  title="Excluir Área"
                >
          ...
          <div className="col-span-full py-12 flex flex-col items-center justify-center text-theme-weak bg-aldebaran-dark border border-dashed border-aldebaran-border">
